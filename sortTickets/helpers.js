function normalize_count(count) {
    return Number.isNaN(Number.parseInt(count)) ||
        Math.floor(count) > 50 ||
        Math.floor(count) < 2
        ? 10
        : Math.floor(count);
...
function discover_the_train_tickets(count) {
    ...
    while (trip_tickets.length > 0) {
        ticket_ix = Math.ceil(Math.random() * trip_tickets.length - 1);
...
function calculate_randomness(ticket_stack) {
    ...
    while (typeof ticket_stack[ix + 1] !== "undefined") {
        if (
            ticket_stack[ix]["to"].toLowerCase() !==
            ticket_stack[ix + 1]["from"].toLowerCase()
        ) {
            random_count++;
        }
        ix++;
